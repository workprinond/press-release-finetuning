# # bmw_comparative_finetuning.py - STRETCH VERSION (Option 1.2)
# # Complete implementation for comparative fine-tuning of original vs reduced DistilGPT2 models
...
#         # Create datasets
#         self.train_dataset = BMWDataset(train_texts, self.tokenizer)
#         self.val_dataset = BMWDataset(val_texts, self.tokenizer)
#         self.test_dataset = BMWDataset(test_texts, self.tokenizer)
        
#         return self.train_dataset, self.val_dataset, self.test_dataset
    
#     # def train_models(self, num_epochs=10, batch_size=8, learning_rate=1e-2):
#     #     """Train both models with identical settings"""
#     #     logger.info(f"Starting comparative training for {num_epochs} epochs")
...
#     #     # Training results
#     #     training_results = {
#     #         'original': {'train_losses': [], 'val_losses': [], 'times': []},
#     #         'reduced': {'train_losses': [], 'val_losses': [], 'times': []}
#     #     }
...
#     #         # Create new optimizer and scheduler for each epoch to ensure identical settings
...
#     #         logger.info(f"Original Model - Train Loss: {original_train_loss:.4f}, "
#     #                    f"Val Loss: {original_val_loss:.4f}, Time: {original_time:.1f}s")
#     #         logger.info(f"Reduced Model  - Train Loss: {reduced_train_loss:.4f}, "
#     #                    f"Val Loss: {reduced_val_loss:.4f}, Time: {reduced_time:.1f}s")
#     #         logger.info(f"Speedup: {(original_time - reduced_time) / original_time * 100:.1f}% faster")
...
#                     early_stopping_patience=3, min_delta=0.001):
...
#         logger.info(f"Early stopping patience: {early_stopping_patience} epochs, Min delta: {min_delta}")
...
#         # Training results
#         training_results = {
#             'original': {'train_losses': [], 'val_losses': [], 'times': [], 'epochs_completed': 0},
#             'reduced': {'train_losses': [], 'val_losses': [], 'times': [], 'epochs_completed': 0}
#         }
        
#         # Early stopping tracking
#         best_original_val_loss = float('inf')
#         best_reduced_val_loss = float('inf')
#         original_patience_counter = 0
#         reduced_patience_counter = 0
...
#             # Create new optimizer and scheduler for each epoch
#             original_optimizer = torch.optim.AdamW(
#                 [p for p in self.original_model.parameters() if p.requires_grad],
#                 lr=learning_rate,
#                 weight_decay=0.01
...
#             if original_val_loss < best_original_val_loss - min_delta:
#                 best_original_val_loss = original_val_loss
#                 original_patience_counter = 0
#                 logger.info(f"Original model improved: {original_val_loss:.4f} (best: {best_original_val_loss:.4f})")
#             else:
#                 original_patience_counter += 1
#                 logger.info(f"Original model no improvement: {original_patience_counter}/{early_stopping_patience}")
            
#             if reduced_val_loss < best_reduced_val_loss - min_delta:
#                 best_reduced_val_loss = reduced_val_loss
#                 reduced_patience_counter = 0
#                 logger.info(f"Reduced model improved: {reduced_val_loss:.4f} (best: {best_reduced_val_loss:.4f})")
#             else:
#                 reduced_patience_counter += 1
#                 logger.info(f"Reduced model no improvement: {reduced_patience_counter}/{early_stopping_patience}")
            
#             # Log epoch results
#             logger.info(f"\nEpoch {epoch + 1} Results:")
#             logger.info(f"Original - Train: {original_train_loss:.4f}, Val: {original_val_loss:.4f}, "
#                     f"Best: {best_original_val_loss:.4f}, Patience: {original_patience_counter}/{early_stopping_patience}")
#             logger.info(f"Reduced  - Train: {reduced_train_loss:.4f}, Val: {reduced_val_loss:.4f}, "
#                     f"Best: {best_reduced_val_loss:.4f}, Patience: {reduced_patience_counter}/{early_stopping_patience}")
...
#             if original_patience_counter >= early_stopping_patience:
#                 logger.info(f"Original model early stopping triggered after {epoch+1} epochs")
#                 stop_training = True
#                 stop_reason = "original model converged"
                
#             if reduced_patience_counter >= early_stopping_patience:
#                 logger.info(f"Reduced model early stopping triggered after {epoch+1} epochs")
#                 stop_training = True
#                 stop_reason = "reduced model converged"
                
#             if stop_training:
#                 logger.info(f"Stopping training early: {stop_reason}")
#                 break
...
#                 f"Final val loss: {training_results['original']['val_losses'][-1]:.4f}")
#         logger.info(f"Reduced model: {training_results['reduced']['epochs_completed']} epochs, "
#                 f"Final val loss: {training_results['reduced']['val_losses'][-1]:.4f}")
...
#             original_gen = self.original_trainer.generate_text(prompt, max_length=80)
#             reduced_gen = self.reduced_trainer.generate_text(prompt, max_length=80)
...
#         # 3. Optional: Evaluate on Q&A set if provided
#         qa_results = None
#         qa_set_path = "./bmw_qa_eval.json"
#         if os.path.exists(qa_set_path):
#             logger.info("\nEvaluating on BMW Q&A set...")
#             qa_results = self.evaluate_qa_set(qa_set_path)
...
#         logger.info(f"Performance Difference: "
#                    f"{(reduced_perplexity - original_perplexity) / original_perplexity * 100:.1f}%")
        
#         return eval_results
    
#     def evaluate_qa_set(self, qa_set_path):
#         """
#         Optional: Evaluate models on BMW Q&A set
        
#         This evaluates the models' ability to answer BMW-related questions
#         by computing average log-likelihood of the correct answers.
#         """
#         try:
...
#                 raise ValueError("Q&A data should be a list of dictionaries")
...
#             avg_orig_ll = np.mean(original_log_likelihoods)
#             avg_red_ll = np.mean(reduced_log_likelihoods)
...
#             logger.warning(f"Could not evaluate Q&A set: {e}")
#             return None
...
#         # Save generations separately for readability
...
#             axes[0, 0].plot(epochs, training_results['original']['train_losses'], 
#                            'b-', label='Original', linewidth=2)
#             axes[0, 0].plot(epochs, training_results['reduced']['train_losses'], 
...
#             axes[0, 1].plot(epochs, training_results['original']['val_losses'], 
#                            'b-', label='Original', linewidth=2)
#             axes[0, 1].plot(epochs, training_results['reduced']['val_losses'], 
#                            'r-', label='Reduced', linewidth=2)
...
#                 axes[1, 1].bar(x - width/2, loss_values, width, label='Test Loss')
#                 axes[1, 1].bar(x + width/2, perplexity_values, width, label='Perplexity')
...
#                 for epoch in range(len(training_results['original']['train_losses'])):
#                     f.write(f"\nEpoch {epoch + 1}:\n")
#                     f.write(f"  Original - Train Loss: {training_results['original']['train_losses'][epoch]:.4f}, "
#                            f"Val Loss: {training_results['original']['val_losses'][epoch]:.4f}, "
#                            f"Time: {training_results['original']['times'][epoch]:.1f}s\n")
#                     f.write(f"  Reduced  - Train Loss: {training_results['reduced']['train_losses'][epoch]:.4f}, "
#                            f"Val Loss: {training_results['reduced']['val_losses'][epoch]:.4f}, "
#                            f"Time: {training_results['reduced']['times'][epoch]:.1f}s\n")
                
#                 total_orig_time = np.sum(training_results['original']['times'])
#                 total_red_time = np.sum(training_results['reduced']['times'])
#                 speedup = (total_orig_time - total_red_time) / total_orig_time * 100
...
#     """
#     Helper function to create a simple BMW Q&A evaluation set.
#     Call this function to generate the Q&A JSON file.
#     """
#     qa_examples = [
#         {
...
#     with open("./bmw_qa_eval.json", "w") as f:
#         json.dump(qa_examples, f, indent=2)
    
#     logger.info("Q&A dataset created: bmw_qa_eval.json")
#     return qa_examples

# def run_comparative_pipeline():
#     """Main function to run the complete comparative pipeline"""
#     print("="*70)
#     print("BMW COMPARATIVE FINE-TUNING PIPELINE (Option 1.2)")
#     print("="*70)
...
#     # Create optional Q&A dataset
#     print("\n3. Creating optional Q&A evaluation set...")
#     create_qa_dataset()
    
#     # Train models
#     print("\n4. Starting comparative training (this may take 2-4 hours)...")
...
#         perf_diff = ((test_metrics['reduced']['perplexity'] - 
#                      test_metrics['original']['perplexity']) / 
#                     test_metrics['original']['perplexity'] * 100)
...
#             speedup = (total_orig - total_red) / total_orig * 100
#             print(f"  Training Speedup: {speedup:.1f}% faster")
...
# bmw_comparative_finetuning.py - ENHANCED STRETCH VERSION
# Complete implementation with early stopping, improved metrics, and Q&A evaluation
...
#     def compute_text_quality_metrics(self):
#         """Compute BLEU and ROUGE scores for generated text."""
#         try:
#             logger.info("Computing text quality metrics (BLEU/ROUGE)...")
            
#             # Load metrics
#             bleu_metric = evaluate.load("bleu")
#             rouge_metric = evaluate.load("rouge")
            
#             # Use test set as references for generation quality
#             references = []
#             predictions_original = []
#             predictions_reduced = []
            
#             # Generate for each test sample (limited to 10 for speed)
#             for i, ref_text in enumerate(self.test_texts[:10]):
#                 # Use first 20 words as prompt
#                 prompt_tokens = self.tokenizer.encode(ref_text, max_length=50, truncation=True)
#                 prompt = self.tokenizer.decode(prompt_tokens, skip_special_tokens=True)
                
#                 # Generate with original model
#                 orig_gen = self.original_trainer.generate_text(prompt, max_length=80, temperature=0.7)
#                 predictions_original.append(orig_gen)
                
#                 # Generate with reduced model
#                 red_gen = self.reduced_trainer.generate_text(prompt, max_length=80, temperature=0.7)
#                 predictions_reduced.append(red_gen)
...
#                 orig_bleu = bleu_metric.compute(predictions=predictions_original, references=references)
#                 orig_rouge = rouge_metric.compute(predictions=predictions_original, references=[r[0] for r in references])
                
#                 red_bleu = bleu_metric.compute(predictions=predictions_reduced, references=references)
#                 red_rouge = rouge_metric.compute(predictions=predictions_reduced, references=[r[0] for r in references])
                
#                 text_metrics = {
#                     'original': {
#                         'bleu': orig_bleu['bleu'],
#                         'rouge1': orig_rouge['rouge1'],
#                         'rouge2': orig_rouge['rouge2'],
#                         'rougeL': orig_rouge['rougeL']
#                     },
#                     'reduced': {
#                         'bleu': red_bleu['bleu'],
#                         'rouge1': red_rouge['rouge1'],
#                         'rouge2': red_rouge['rouge2'],
#                         'rougeL': red_rouge['rougeL']
#                     },
#                     'num_samples_evaluated': len(predictions_original)
#                 }
                
#                 with open(os.path.join(self.output_dir, "text_quality_metrics.json"), 'w') as f:
#                     json.dump(text_metrics, f, indent=2)
                
#                 logger.info(f"Text Quality - Original BLEU: {text_metrics['original']['bleu']:.4f}, ROUGE-L: {text_metrics['original']['rougeL']:.4f}")
#                 logger.info(f"Text Quality - Reduced BLEU: {text_metrics['reduced']['bleu']:.4f}, ROUGE-L: {text_metrics['reduced']['rougeL']:.4f}")
                
#                 return text_metrics
#             else:
#                 logger.warning("No predictions generated for text quality evaluation")
#                 return None
                
#         except Exception as e:
#             logger.error(f"Error computing text quality metrics: {e}")
#             return None
...
#         # 4. Compute text quality metrics
#         logger.info("\nComputing text quality metrics...")
#         text_quality_results = self.compute_text_quality_metrics()
...
#             'generations': generations,
#             'qa_evaluation': qa_results,
#             'text_quality_metrics': text_quality_results
#         }
...
#                 if self.comparison_results['evaluation']['text_quality_metrics']:
#                     text_metrics = self.comparison_results['evaluation']['text_quality_metrics']
#                     f.write("TEXT QUALITY METRICS\n")
#                     f.write("-"*40 + "\n")
#                     f.write(f"Original Model - BLEU: {text_metrics['original']['bleu']:.4f}, "
#                            f"ROUGE-L: {text_metrics['original']['rougeL']:.4f}\n")
#                     f.write(f"Reduced Model  - BLEU: {text_metrics['reduced']['bleu']:.4f}, "
#                            f"ROUGE-L: {text_metrics['reduced']['rougeL']:.4f}\n\n")
...
#     print("  - training_results.csv: Training loss and time data")
#     print("  - evaluation_results.json: Comprehensive evaluation metrics")
#     print("  - text_quality_metrics.json: BLEU/ROUGE scores")
